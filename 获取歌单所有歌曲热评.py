...
def get_all_hotSong():#获取热歌榜所有歌曲名称和id
    U = input('歌单网址http://music.163.com/')  #输入歌单网址 去掉#/
    url='http://music.163.com/{}'.format(U)   #网易云云音乐热歌榜url
    header={    #请求头部
...
def get_hotComments(hot_song_name,hot_song_id):
   ...
   'User-Agent':'Mozilla/5.0 (X11; Fedora; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36'
    ...
    num=0
    fhandle = open('./song_comments', 'a', encoding='utf-8')  # 写入文件 utf-8
    ...
    for hotcommment in data['hotComments'] :
        ...
        num+=1
        fhandle.write(str(num)+'.'+str(item['nickname'])+'  '+str(item['likedCount'])+'-----'+str(item['content'])+'\n')
    fhandle.write('\n==============================================\n\n')
    fhandle.close()
